describe Luffa::RetryOpts do
  ...
  describe '.tries_and_intervals' do
    ...
    describe 'does not all some options' do
      let(:retry_module) { Luffa::RetryOpts }
    ...
    it 'other options are allowed' do
      ...
      actual = Luffa::RetryOpts.tries_and_interval(3, 4, other_options)
      expect(actual).to be == {:tries => 3,
                               :interval => 4,
                               :some_other_key => 'a'}
